	...
	: legion(legion), CHANNELS(legion.CHANNELS), FRAMES(legion.FRAMES)
{
	...
	int alive = 0;
	int match = 0;
	GroupingOscillator* deadOscillator = new GroupingOscillator();
	for (int channel = 0; channel < CHANNELS; channel++) {
		...
		for (int frame = 0; frame < FRAMES; frame++) {
			...
			if (segNum >= 0) {
				...
				if ((*legion.correlogram.T_FGrid)[frame][channel][time] / (*legion.correlogram.T_FGrid)[frame][channel][0] > .95)  {
					match++;
